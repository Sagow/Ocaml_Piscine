...
let rec helix_of_rna r =
  match r with
  | [] -> []
  | head::queue ->
    match head.n with
    | A -> (generate_nucleotide 't') :: (helix_of_rna queue)
    | U -> (generate_nucleotide 'a') :: (helix_of_rna queue)
    | C -> (generate_nucleotide 'g') :: (helix_of_rna queue)
    | G -> (generate_nucleotide 'c') :: (helix_of_rna queue)
    | _ -> (generate_nucleotide ' ') :: (helix_of_rna queue)
...
let life name =
  let rna = generate_plausible_rna 9 in
  let helix = helix_of_rna rna in 
  let protein = string_of_protein (decode_arn rna) in
  print_endline ("----" ^ name ^ "----");
  print_endline ("helix : " ^ helix_to_string helix);
  print_endline ("rna   : " ^ helix_to_string rna);
  print_endline ("protein : " ^ protein);